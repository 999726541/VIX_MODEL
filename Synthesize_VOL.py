...
def monthOptsForVol(daysExpecting = 93, calculating = False,cur_date = datetime.today().strftime("%Y-%m-%d %T"),whole_table = None):
    '''
    ...
    monthly_option_list = get_existing_month_contract(cur_date,whole_table = whole_table)
    cur_date = datetime.strptime(cur_date, "%Y-%m-%d %H:%M:%S")
    ...
    if target_dates not in monthly_option_list:
        ...
        assert monthly_option_list.index(target_dates)!=0, ValueError('Date is to short or That date is in holiday, '
                                                                      'please enter value larger than 30 days')
...
def VIX_index(nearest_timestamp=datetime.today().strftime("%Y-%m-%d %T"),whole_table = None):
    date_front, date_back = Opts_For_VIX(30, calculating=True, cur_date=nearest_timestamp)

    if date_back == None:
        date_front = date_front.strftime("%Y%m%d")[2:]
        _front = vol_caculator([date_front], nearest_timestamp=nearest_timestamp,wholeTable=whole_table)
        ...
        _front = vol_caculator([date_front], nearest_timestamp=nearest_timestamp,wholeTable=whole_table)
        _back = vol_caculator([date_back], nearest_timestamp=nearest_timestamp,wholeTable=whole_table)
...
def linearSpineLine_X_days_vol(days,nearest_timestamp=datetime.today().strftime("%Y-%m-%d %T"),whole_table = None):
    # find which one or two monthly option you need
    date_front, date_back = monthOptsForVol(days, calculating=True, cur_date=nearest_timestamp,whole_table=whole_table)
...
if __name__=='__main__':
    ...
    print(linearSpineLine_X_days_vol(93,'2017-08-03 11:35:00'))
